#include "TwoLinkManipulator.h"
#include "AMPCore.h"
#include <cmath> 

namespace amp{

    TwoLinkManipulator::TwoLinkManipulator(const std::vector<double>& linkLengths) : LinkManipulator2D(linkLengths) {}
    TwoLinkManipulator::TwoLinkManipulator() : LinkManipulator2D() {}
    TwoLinkManipulator::~TwoLinkManipulator() {}

    Eigen::Vector2d TwoLinkManipulator::getJointLocation(const ManipulatorState& state, uint32_t joint_index) const {
    ...
    ManipulatorState TwoLinkManipulator::getConfigurationFromIK(const Eigen::Vector2d& end_effector_location) const {
        ...
        else if (nLinks() == 3)
        {
            ...
            for(double psi = 0; psi <= 2*M_PI; psi += 0.01){
