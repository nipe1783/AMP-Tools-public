...
int main(int argc, char** argv) {
    MyDecentralizedMultiAgentRRT rrt;
    ...
    //     MultiAgentProblem2D prob_1_2m = HW8::getWorkspace1(2);
    //     auto answer_1_2m = rrt.plan(prob_1_2m, true);
    //     times_1_2m.push_back(std::get<1>(answer_1_2m));

    //     // Configuration for 3 agents
    //     MultiAgentProblem2D prob_1_3m = HW8::getWorkspace1(3);
    //     auto answer_1_3m = rrt.plan(prob_1_3m, true);
    //     times_1_3m.push_back(std::get<1>(answer_1_3m));

    //     // Configuration for 4 agents
    //     MultiAgentProblem2D prob_1_4m = HW8::getWorkspace1(4);
    //     auto answer_1_4m = rrt.plan(prob_1_4m, true);
    //     times_1_4m.push_back(std::get<1>(answer_1_4m));

    //     // Configuration for 5 agents
    //     MultiAgentProblem2D prob_1_5m = HW8::getWorkspace1(5);
    //     auto answer_1_5m = rrt.plan(prob_1_5m, true);
    //     times_1_5m.push_back(std::get<1>(answer_1_5m));

    //     // Configuration for 6 agents
    //     MultiAgentProblem2D prob_1_6m = HW8::getWorkspace1(6);
    //     auto answer_1_6m = rrt.plan(prob_1_6m, true);
    //     times_1_6m.push_back(std::get<1>(answer_1_6m));
    // }
    ...
    // std::vector<double> avgTimes = {sum_vector(times_1_2m) / times_1_2m.size(), sum_vector(times_1_3m) / times_1_3m.size(), sum_vector(times_1_4m) / times_1_4m.size(), sum_vector(times_1_5m) / times_1_5m.size(), sum_vector(times_1_6m) / times_1_6m.size()};
    // // std::vector<double> avgNodes = {sum_vector(nodes_1_2m) / nodes_1_2m.size(), sum_vector(nodes_1_3m) / nodes_1_3m.size(), sum_vector(nodes_1_4m) / nodes_1_4m.size(), sum_vector(nodes_1_5m) / nodes_1_5m.size(), sum_vector(nodes_1_6m) / nodes_1_6m.size()};
    // std::vector<double> numAgents = {2, 3, 4, 5, 6};
    

    // std::string title = "Average Time vs. Number of Agents";
    ...
    // Visualizer::makeBarGraph(avgTimes, labels, title, xlabel, "time (ms)");
    // // Visualizer::makeBarGraph(avgNodes, labels, "Average Size of tree vs. Number of Agents", xlabel, "Size of Tree");
    ...
    // Visualizer::makeBoxPlot(dataTimeList, labels, "Time vs. Number of Agents", xlabel, "time (ms)");
    // // Visualizer::makeBoxPlot(dataNodesList, labels, "Size of tree vs. Number of Agents", xlabel, "Size of Tree");
    // Visualizer::showFigures();
